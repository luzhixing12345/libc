...
int main(int argc, const char **argv) {
    ...
    if (ismatch(&parser, "help")) {
        argparse_info(&parser);
    }
    if (ismatch(&parser, "version")) {
        printf("v0.0.1\n");
    }
    if (ismatch(&parser, "input")) {
        printf("i = %d\n", integer);
    }
    if (ismatch(&parser, "string")) {
        printf("s = %s\n", str);
    }
    if (ismatch(&parser, "dest")) {
        printf("dest = %s\n", dest);
    }
    if (ismatch(&parser, "src")) {
        printf("src = %d\n", src);
    }
    int n = ismatch(&parser, "other-number");
    for (int i = 0; i < n; i++) {
...
以 `ARG*` 开头的是宏, 第一个位置用于参数绑定, 后面的内容会被展开为字符串用于后续的解析
...
上例中的每一个选项中都使用了 NAME, 这并不是一个必选项, 除了使用 is_match 来判断是否接收到了参数, 也可以判断绑定的参数的值.

## 接收参数

`ismatch` 函数用于判断是否接收到了参数, 并返回匹配的个数. 第二个参数是您定义的名字或长参数的名字

```c
int ismatch(argparse *parser, char *name);
int match_pos(argparse *parser, char *name);
```
...
xbox 提供了一个函数 `argparse_info` 用于帮助信息的输出, 建议您在 options 中添加 -h 选项并将其绑定到 argparse_info 函数, 用于辅助帮助信息的输出, 当然您也可以编写您自定义的 help 信息文档. 默认的帮助信息会自动为您实现对齐和说明
...
**xargparse 使用了动态内存分配, 所以最后请注意使用 `free_argparse` 释放内存**
...
int main(int argc, const char **argv) {
    ...
    int n = ismatch(&parser, "src");
    for (int i = 0; i < n; i++) {
        printf("file[%d] = [%s]\n", i, files[i]);
    }

    n = ismatch(&parser, "include");
    for (int i = 0; i < n; i++) {
        printf("include path[%d] = [%s]\n", i, include_path[i]);
    }

    n = ismatch(&parser, "library_path");
    for (int i = 0; i < n; i++) {
        printf("library path[%d] = [%s]\n", i, library_path[i]);
    }

    n = ismatch(&parser, "library_name");
    for (int i = 0; i < n; i++) {
